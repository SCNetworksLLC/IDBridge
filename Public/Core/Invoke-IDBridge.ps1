function Invoke-IDBridge {
    ...
    try {
        $global:IDConfig = Get-IDBridgeConfiguration -RootPath $RootPath -ErrorAction Stop
    }
    catch { Throw $_ }
    #endregion Import Configuration

    #region Apply Runtime Overrides
    if ($PSBoundParameters.ContainsKey('ReadOnly'))       { $global:IDConfig.Debug.readOnly       = [bool]$ReadOnly }
    if ($PSBoundParameters.ContainsKey('TestRun'))        { $global:IDConfig.Debug.testRun        = [bool]$TestRun }
    if ($PSBoundParameters.ContainsKey('SkipADCheck'))    { $global:IDConfig.Debug.skipADCheck    = [bool]$SkipADCheck }
    if ($PSBoundParameters.ContainsKey('VerboseLogging')) { $global:IDConfig.Debug.verboseLogging = [bool]$VerboseLogging }
    if ($SkipAD)      { $global:IDConfig.AD.enabled     = $false; $global:IDConfig.AD.enableGroupProcessing     = $false }
    if ($SkipGoogle)  { $global:IDConfig.Google.enabled = $false; $global:IDConfig.Google.enableGroupProcessing = $false }
    ...
    $logSheetID     = $IDConfig.Logging.SheetID
    ...
    foreach ($plugin in $IDConfig.Plugins) {
        if ($plugin.Enabled -ne $true) {
            if ($verboseLogging -eq $true) {
                Write-Log -Message "Plugin: $($plugin.Function) is disabled in config. Skipping plugin." -Level Info
            }
            Continue
        }
        ...
        catch { Throw (Start-ScriptEnd -UploadLogsSheetID $logSheetID -GoogleHeaders $headersGoogle -Message $_ -WriteError) }
        ...
        if ($pluginData) {
            ...
            Write-Log -Message "Plugin: $($plugin.Function) did not return any data." -Level Warn
        }
    }

    if ($sourceData.Count -eq 0) {
        Throw (Start-ScriptEnd -UploadLogsSheetID $logSheetID -GoogleHeaders $headersGoogle -Message "No source data gathered from plugins. Please check plugin configurations and logs for details." -WriteError)
    }
    ...
    if ($IDConfig.Google.enabled -eq $true) {
        try {
            $googleData = Get-TargetDataGoogle -headers $headersGoogle -VerboseLogging $verboseLogging -ErrorAction Stop
        }
        catch { Throw (Start-ScriptEnd -UploadLogsSheetID $logSheetID -GoogleHeaders $headersGoogle -Message $_ -WriteError) }
    }
    ...
    if ($IDConfig.AD.enabled -eq $true) {
        try {
            $adData = Get-TargetDataAD -VerboseLogging $verboseLogging -ErrorAction Stop
        }
        catch { Throw (Start-ScriptEnd -UploadLogsSheetID $logSheetID -GoogleHeaders $headersGoogle -Message $_ -WriteError)}
    }
    ...
    if ($sourceData | Where-Object {$_.ADDuplicateIDStatus -or $_.GoogleDuplicateIDStatus}) {
        foreach ($item in $sourceData | Where-Object {$_.ADDuplicateIDStatus -or $_.GoogleDuplicateIDStatus}) {
            Write-Log -Message ("Removing User from Processing with Duplicate Person ID: $($item.personID) - UPN: $($item.UPN) - AD Duplicate Status: $($item.ADDuplicateIDStatus) - Google Duplicate Status: $($item.GoogleDuplicateIDStatus)") -Level Info
        }
    ...
    if ($duplicateIDs) {
        foreach ($item in $sourceData | Where-Object { $_.personID -in $duplicateIDs }) {
            Write-Log -Message ("Removing User from Processing with Duplicate Person ID: $($item.personID) - UPN: $($item.UPN) - IDBridge Duplicate Status: DUPLICATE_ID") -Level Info
        }
    ...
    #AD Checks
    if ($IDConfig.AD.enabled -eq $true -and ($IDConfig.AD.enableGroupProcessing -eq $true -or $IDConfig.AD.enableGroupProcessingWhatIf -eq $true) -and $verboseLogging -eq $true) {
        Show-GroupsNotProcessedAD -UserList $sourceData -CurrentADGroups $adData.Groups    }

    #Google Checks
    if ($IDConfig.Google.enabled -eq $true -and ($IDConfig.Google.enableGroupProcessing -eq $true -or $IDConfig.Google.enableGroupProcessingWhatIf -eq $true) -and $verboseLogging -eq $true) {
        Show-GroupsNotProcessedGoogle -UserList $sourceData -CurrentGoogleGroups $googleData.Groups    }
    ...
    if ($IDConfig.AD.enabled -eq $true) {
        ...
        foreach ($item in $sourceData | Where-Object {$ADUsersToSetEmployeeID.ContainsKey($_.personID)}) {
                Write-Log -Message ("AD: Matched $($ADUsersToSetEmployeeID[$item.personID].User.UserPrincipalName) with EmployeeID: $($item.personID).")
    ...
    if ($IDConfig.Google.enabled -eq $true) {
        ...
        foreach ($item in $sourceData | Where-Object {$GoogleUsersToSetEmployeeID.ContainsKey($_.personID)}) {
                Write-Log -Message ("Google: Matched $($GoogleUsersToSetEmployeeID[$item.personID].User.primaryEmail) with EmployeeID: $($item.personID).")
    ...
    if ($IDConfig.AD.enabled -eq $true -and $IDConfig.Debug.readOnly -eq $false) {
        ...
        foreach ($item in $ADOrgUnitsForProcessing | Sort-Object -Unique) {
            try {
                ...
                Throw (Start-ScriptEnd -UploadLogsSheetID $logSheetID -GoogleHeaders $headersGoogle -Message $_ -WriteError)
            }
        }
        ...
        if ($IDConfig.AD.enableGroupProcessing -eq $true) {
            ...
            if ($ADUsersToCreate.Count -gt 0) {
                ...
                if ($verboseLogging -eq $true) {
                    Write-Log -Message "AD: Refreshing AD User Groups to Update List to include newly created users." -Level Info
                }
                $ADUserGroupsToUpdate = Get-ADUserGroupsToUpdate -UserList $sourceData -CurrentADGroups $adData.Groups            }
    ...
    if ($IDConfig.Google.enabled -eq $true -and $IDConfig.Debug.readOnly -eq $false) {
        ...
        foreach ($item in $GoogleOrgUnitsForProcessing) {
            try {
                New-IDBridgeGoogleOrgUnit -OrgUnit $item -tokenInformation $headersGoogle            }
            catch {
        ...
        foreach ($item in $GoogleUsersToDeactivate) {
            try {
                ...
                Update-IDBridgeGoogleUser -GoogleUserID $item.GoogleCurrentUserID -OrgUnitPath $item.GoogleOrganizationalUnitTrash -Suspended 'true' -tokenInformation $headersGoogle            }
            catch {
                Write-Log -Message $_ -Level Error
            }
            ...
            if ($IDConfig.Google.enableGroupProcessing -eq $true -and $IDConfig.Google.enableGroupProcessingTrash -eq $true) {
                foreach ($group in $item.GoogleCurrentGroups) {
                    try {
                        ...
                        Update-GoogleGroupMembers -GroupEmail $group -PersonID $item.GoogleCurrentUserID -UpdateType "Remove" -TokenInformation $headersGoogle                    }
                    catch {
                        Write-Log -Message $_ -Level Error
                    }
                }
        ...
        foreach ($item in $GoogleUsersToUpdate) {
            try {
                ...
                $itemSplat = $null
                $itemSplat = $item.splat
                Update-IDBridgeGoogleUser @itemSplat -tokenInformation $headersGoogle            }
            catch {
                Write-Log -Message $_ -Level Error
            }
        }
        ...
        foreach ($item in $GoogleUsersToCreate) {
            try {
                ...
                $newUserResponse = New-IDBridgeGoogleUser @itemSplat -tokenInformation $headersGoogle -ErrorAction Stop
        ...
        if ($IDConfig.Google.enableGroupProcessing -eq $true) {
            if ($GoogleUsersToCreate.Count -gt 0) {
                ...
                if ($verboseLogging -eq $true) {
                    Write-Log -Message "Google: Refreshing AD User Groups to Update List to include newly created users." -Level Info
                }
                $GoogleUserGroupsToUpdate = Get-GoogleUserGroupsToUpdate -UserList $sourceData -GoogleGroups $googleData.Groups -GroupPrimaryDomainName $IDConfig.Google.GroupPrimaryDomainName            }
            ...
            foreach ($item in $GoogleUserGroupsToUpdate.Add) {
                foreach ($group in $item.Groups) {
                    try {
                        ...
                        Update-GoogleGroupMembers -GroupEmail ($googleData.Groups | Where-Object {$_.name -eq $group}).email -PersonID $item.GoogleCurrentUserID -UpdateType "Add" -TokenInformation $headersGoogle                    }
                    catch {
                        Write-Log -Message $_ -Level Error
                    }
                }
    ...
    Start-ScriptEnd -UploadLogsSheetID $logSheetID -GoogleHeaders $headersGoogle
}